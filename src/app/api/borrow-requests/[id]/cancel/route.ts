...
import { borrowRequestCancelSchema } from "@/lib/validators/borrow-request-actions";
...
export const POST = withErrorHandler(
  async (
    ...
    const input = await parseJsonBody(request, borrowRequestCancelSchema);
    const { borrowRequestService } = createBorrowStack();
    const borrowRequest = await borrowRequestService.cancelBorrowRequest(actor, id, input);
