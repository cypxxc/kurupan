...
export function ReturnsPageClient({
  ...
  initialPage: PaginatedResult<ReturnTransaction>;
}) {
  const { user } = useAuth();
  ...
  const returns = initialPage.items;
            ...
            <TableBody>
              {isPending ? (
                ...
                returns.map((transaction) => {
                  const hasLost = transaction.items.some((item) => item.condition === "lost");
                  const hasDamaged = transaction.items.some((item) => item.condition === "damaged");
                  const condition = hasLost ? "lost" : hasDamaged ? "damaged" : "good";
